...
from .utils import make_conv_layer, make_subsample_layers

BN_MOMENTUM = 0.1


def build(cfg, pretrained=True, **kwargs):
    hr_net_cfg = cfg.get('hrnet')
    model = HighResolutionNet(hr_net_cfg, **kwargs)

    pretrained_path = hr_net_cfg.get('pretrained_path')
    if pretrained:
...
class HighResolutionNet(nn.Module):
    def __init__(self):
        ...
        self.subsample_3 = self._make_subsample_layer(
            in_channels=stage2_num_channels[-1],
            num_layers=2)
        self.subsample_2 = self._make_subsample_layer(
            in_channels=stage3_num_channels[-1],
            num_layers=1)
    ...
    def load_weights(self, pretrained=''):
        ...
        if osp.isfile(pretrained):
            ...
            logger.warning('=> please download pre-trained models first!')
            logger.warning(f'{pretrained} does not exist!')
